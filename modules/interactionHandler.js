...
async function handleInteraction(interaction) {
    try {
        ...
        // Handle broadcast logs pagination buttons
        if (interaction.isButton() && interaction.customId.startsWith('blpg:')) {
        ...
        switch (commandName) {
            ...
            case 'broadcast':
                ...
                break;
            case 'broadcastlogs':
                await handleBroadcastLogs(interaction);
                break;
    ...
    try {
        ...
        for (const guildConfig of guilds) {
            ...
            try {
                ...
                failCount++;
                failedGuilds.push({ guildId: guildConfig.guildId, reason: error.message || 'Unknown error' });
            }
        }
...
async function handleBroadcastLogs(interaction) {
    ...
    try {
        ...
        if (allLogs.length === 0) {
            await interaction.editReply('No broadcast logs found.');
            return;
        }
        ...
        await interaction.editReply('An error occurred while fetching broadcast logs.');
    }
}

async function handleBroadcastLogsPagination(interaction) {
    // Only the bot owner's buttons should work
    if (interaction.user.id !== process.env.BOT_OWNER_ID) {
    ...
    try {
        ...
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'An error occurred while paginating.', flags: MessageFlags.Ephemeral });
        }
    }
}
